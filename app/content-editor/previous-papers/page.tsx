...
export default async function ContentEditorPreviousPapersPage() {
  try {
  ...
  return <ContentEditorLayout><PageHeader title="Previous official papers" description="Create, edit, publish, and archive official exam paper references." /><PreviousOfficialPaperManager papers={papers} courses={courses.map((course) => ({ id: course.id, title: course.title }))} modules={modules.map((module) => ({ id: module.id, title: module.title, courseId: module.courseId }))} /></ContentEditorLayout>
}

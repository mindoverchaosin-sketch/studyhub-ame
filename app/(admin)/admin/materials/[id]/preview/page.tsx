...
export default async function StudyMaterialPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6"><PageHeader title={`Preview: ${resource.title}`} description="Preview uses the same renderer as the student view." /><div className="flex flex-wrap gap-3"><a href={`/api/study-materials/${resource.id}/pdf?preview=1`} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white">Download PDF</a></div><StudyMaterialWebRenderer document={previewDocument} /></div>
}
